...
%             msgbin = randint(b,N);   % binary squences
            msgbin = randi(2, b,N) -1;   % binary squences
...
subplot(121), semilogy(z,CCDF_no), grid on, hold on
for cr = 1:N_CR
   gs = gss(cr);
   subplot(121), semilogy(z,CCDF_c(cr,:),[gs '-'], z,CCDF_cf(cr,:),[gs ':']), hold on
   subplot(122), semilogy(SNRdBs,ber_c(cr,:),[gs '-'], SNRdBs,ber_cf(cr,:),[gs ':']), hold on
end 
